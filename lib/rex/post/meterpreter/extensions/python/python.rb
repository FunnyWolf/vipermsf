...
class Python < Extension
  ...
  # toybox
  def import(file, mod_name, result_var, timeout = 7200)
    ...
    run_exec_request(request,timeout)
  end
  ...
  def execute_string(code, result_var, timeout = 7200)
    request = Packet.create_request(COMMAND_ID_PYTHON_EXECUTE)
    request.add_tlv(TLV_TYPE_PYTHON_CODE, code)
...
private
  ...
  def run_exec_request(request,timeout = 7200)
