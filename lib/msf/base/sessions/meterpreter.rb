...
class Meterpreter < Rex::Post::Meterpreter::Client
  ...
  def bootstrap(datastore = {}, handler = nil)
    ...
    init_session = Proc.new do
      ...
      unless datastore['AutoVerifySession'] == false
        result = session.core.negotiate_tlv_encryption(datastore['AutoVerifySessionTimeout'].to_i)
        if result[:timeout]
  ...
  def load_python
    unless self.ext.aliases.has_key?('python')
      original               = console.disable_output
      console.disable_output = true
      console.run_single('load python')
      console.disable_output = original
    end
  end
  ...
  #
  def load_powershell
    unless self.ext.aliases.has_key?('powershell')
      original               = console.disable_output
      console.disable_output = true
      console.run_single('load powershell')
      console.disable_output = original
    end
  end
  # toybox
  ...
  def update_session_info
    ...
    # toybox
    self.advanced_info = {:username=>username,:sysinfo=>sysinfo}
