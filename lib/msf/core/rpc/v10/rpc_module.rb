...
class RPC_Module < RPC_Base
  ...
  def rpc_execute(mtype, mname, opts, runasjob = true, timeout = 7200)
    ...
    when 'exploit'
      begin
        Timeout.timeout(timeout) do
          result = _run_exploit(mod, opts, runasjob)
          return result
        end
      rescue ::Exception => e
        error(500, e.message)
    ...
    when 'auxiliary'
      begin
        Timeout.timeout(timeout) do
          result = _run_auxiliary(mod, opts, runasjob)
          return result
        end
      rescue ::Exception => e
        error(500, e.message)
    ...
    when 'payload'
      result = _run_payload(mod, opts)
      return result
    when 'post'
      begin
        Timeout.timeout(timeout) do
          result = _run_post(mod, opts, runasjob)
          return result
        end
      rescue ::Exception => e
        error(500, e.message)
...
private
  ...
  def _run_exploit(mod, opts, runasjob = true)
    if runasjob
      ...
      outputio = Rex::Ui::Text::Output::Buffer.new
      inputio  = Rex::Ui::Text::Input::Readline.new
      Msf::Simple::Exploit.exploit_rpc(mod, {
              'Payload'     => opts['PAYLOAD'],
              'Target'      => opts['TARGET'],
              'RunAsJob'    => false,
              'LocalOutput' => outputio,
              'LocalInput'  => inputio,
              'Options'     => opts
      })
    end
  ...
  def _run_auxiliary(mod, opts, runasjob = true)
    if runasjob
      Msf::Simple::Auxiliary.run_rpc(mod, {
              'Action'   => opts['ACTION'],
              'RunAsJob' => true,
              'Options'  => opts
      })
      ...
      outputio = Rex::Ui::Text::Output::Buffer.new
      output   = Msf::Simple::Auxiliary.run_rpc(mod, {
              'Action'      => opts['ACTION'],
              'RunAsJob'    => false,
              'LocalOutput' => outputio,
              'Options'     => opts
  ...
  def _run_post(mod, opts, runasjob = true)
    if runasjob
      Msf::Simple::Post.run_rpc(mod, {
              'RunAsJob' => true,
              'Options'  => opts
      })
      {
