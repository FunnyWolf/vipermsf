...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    print_status("Creating looted pem files for #{rhost}:#{rport}")
    ...
    # print_good("crt: #{p}")
    filename = "#{datastore['RHOSTS'].downcase}_#{datastore['RPORT']}.pem"
    p = store_viper(combined,filename)
    print_good_redis("Store pem on: #{filename}")
