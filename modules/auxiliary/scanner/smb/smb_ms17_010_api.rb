  ...
  def run_host(ip)
    ...
    begin
      ...
      if status == 'STATUS_INSUFF_SERVER_RESOURCES'
        ...
        @process_result = {:host => ip, :result => "VULNERABLE", :os => os}
        return
        ...
        @process_result = {:host => ip, :result => "UNVULNERABLE"}
      else
        print_error("Unable to properly detect if host is vulnerable.")
        @process_result = {:host => ip, :result => "UNDETECT"}
      end
      ...
      @process_result = {:host => ip, :result => "UNREACHABLE"}
    rescue ::Exception => e
