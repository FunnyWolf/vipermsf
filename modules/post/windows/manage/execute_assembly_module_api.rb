...
class MetasploitModule < Msf::Post
  ...
  def initialize(info = {})
    ...
    register_advanced_options(
            [
                    OptBool.new('KILL', [true, 'Kill the injected process at the end of the task', true])
            ]
    )
  end
  ...
  def run
    ...
    # 定位需要的exe
    exe_path = nil
    if File.file?(File.join(Msf::Config.install_root, "scripts", "csharp", "#{datastore['ASSEMBLY']}.exe"))
      exe_path = File.join(Msf::Config.install_root, "scripts", "csharp", "#{datastore['ASSEMBLY']}.exe")
    end
    if File.file?(File.join(Msf::Config.loot_directory, "#{datastore['ASSEMBLY']}.exe"))
      exe_path = File.join(Msf::Config.loot_directory, "#{datastore['ASSEMBLY']}.exe")
    end
    ...
    if exe_path == nil
      pub_json_result(false,
                      "#{datastore['ASSEMBLY']} ASSEMBLY not found",
                      nil,
                      self.uuid)
      return
    end
  ...
  def get_assembly_filepath(installed_dotnet_versions)
    # 定位需要的exe
    exe_path = "#{datastore['ASSEMBLY']}.exe"
    installed_dotnet_versions.each do |fi|
