...
#>
    ...
    if (Test-Path -Path $OutFile)
    {
        ...
        $ObjectCSV | ForEach-Object { $Start = $True }{ if ($Start)
        {
            $Start = $False
        }
        else
        {
            $_
        } } | Out-File -Encoding 'ASCII' -Append -FilePath $OutFile
    }
    else
    {
...
#>
    ...
    if ($Domain)
    {
        ...
        {
            Write-Warning '[!] Add User failed or User already exists!'
            return
        }
    }
    else
    ...
    Get-NetGPO -GPOName $GPOname -DisplayName $DisplayName -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -PageSize $PageSize | ForEach-Object {
        ...
        if ($Inf -and ($Inf.psbase.Keys -contains 'Group Membership'))
        {
            ...
            ForEach ($Membership in $Inf.'Group Membership'.GetEnumerator())
            {
                ...
                if ($MembershipValue -isnot [System.Array])
                {
...
#>
    ...
    $GPOgroups | ForEach-Object {
        ...
        Get-NetOU -Domain $Domain -DomainController $DomainController -GUID $GPOguid -FullData -PageSize $PageSize | ForEach-Object {
            ...
            if ($OUComputers)
            {
                if ($OUComputers -isnot [System.Array])
                {
...
#>
    ...
    begin {
        ...
        $SearchTerms = $SearchTerms | ForEach-Object { if ($_ -notmatch '^\*.*\*$')
        {
            "*$( $_ )*"
        }
        else
    ...
    process {
        ...
        Get-ChildItem @SearchArgs | ForEach-Object {
            ...
            if (!$ExcludeFolders -or !$_.PSIsContainer)
            {
                $_
            }
        } | ForEach-Object {
            ...
            {
                if ($LastAccessTime -and ($_.LastAccessTime -gt $LastAccessTime))
                {
                    $_
                }
                elseif($LastWriteTime -and ($_.LastWriteTime -gt $LastWriteTime))
                {
                    $_
                }
                elseif($CreationTime -and ($_.CreationTime -gt $CreationTime))
                {
                    $_
                }
            }
            ...
            if ((-not$CheckWriteAccess) -or (Invoke-CheckWrite -Path $_.FullName))
            {
                $_
            }
            ...
            if ($OutFile)
            {
                Export-PowerViewCSV -InputObject $_ -OutFile $OutFile
            }
            else
            {
                $_
    ...
    process {
        ...
        else
        {
            if (-not$NoPing -and ($ComputerName.count -ne 1))
            {
                ...
                $Ping = { param($ComputerName) if (Test-Connection -ComputerName $ComputerName -Count 1 -Quiet -ErrorAction Stop)
                {
                    $ComputerName
                } }
...
#>
    ...
    begin {
        ...
        if (!$NoClobber)
        {
            if ($OutFile -and (Test-Path -Path $OutFile))
            {
